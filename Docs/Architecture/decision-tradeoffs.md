...
| 0012 (Proposed) | Evaluate .NET 10 LTS against the exact pinned Fantasy server composition | Infer support from `Fantasy.Net` alone; start a new .NET 8/9 Host; publish three Host TFMs | The local Fantasy server graph is not end-to-end `net10.0`; older runtimes expire soon; multi-publish adds unowned variance | Accept only after full build/publish/start, replay, load, and operational evidence |
