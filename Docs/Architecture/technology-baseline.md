...
In the locally reviewed Fantasy commit `6df3507b15737d86f93a36af1a6d28a2404e163d`, the core `Fantasy.Net` project conditionally exposes `net10.0`, but the Server Main/Entity/Hotfix projects, server templates, Benchmark, and bundled DotRecast projects target only .NET 8 and/or .NET 9. A [Windows compatibility Spike and local candidate migration](runtime-successor-spike-2026-08-13.md) produced local commit `c6c3f06f4ae54af122b24bac1d7dd1048445123b`, which retargets the supported composition, fixes duplicate config publication, builds and starts cleanly, and emits a .NET 10-only package. The commit is not remotely fetchable and Linux/CI, automated tests, replay/load, and operational evidence remain open, so end-to-end support is not yet proven and must pass ADR-0012 before adoption. The repository license is based on the MIT text but adds an entity-specific restriction. Legal/license review remains a release gate before commercial distribution, redistribution of the fork, or publication of derived packages.
