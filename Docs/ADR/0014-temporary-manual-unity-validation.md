...
The project owner selected macOS Apple Silicon ARM64 Mono as the current complete desktop gate because no Windows environment is available. The Windows script remains a supplemental future platform gate. The WS-27 macOS bundle includes the local deterministic Regional correction profile, but does not claim Windows, Universal/x86_64, Android/iOS IL2CPP, physical-WAN behavior, or game-specific prediction physics.
