...
The [two-room capacity contract](../Architecture/multi-room-capacity-validation.md) keeps the production default at one room and exposes the 128-client topology only through evaluation configuration. Exact-`main` [run 32976156874](https://github.com/rayss1/ai-native-unity-framework/actions/runs/32976156874) passed the 128-client Tick, Gameplay, CPU, memory, connection, input-rate, bandwidth, datagram, and ADR-0015 room-aware replay gates with the required 20% headroom and exact final Tick/combined-hash verification. The environment canary with named candidate/fallback digests and operator authority remains an independent gate; no result here authorizes deployment or changes the production default.
