...
The [two-room capacity contract](../Architecture/multi-room-capacity-validation.md) keeps the production default at one room and exposes the 128-client topology only through evaluation configuration. It requires 20% Tick, CPU, memory, and per-client wire headroom and rejects multi-room use of the roomless version 1 replay format. Exact-`main` evidence must pass before production density can change; the environment Canary remains independent and manual.
