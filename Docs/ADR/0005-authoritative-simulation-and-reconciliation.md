...
WS-28 adds an engine-neutral, presentation-only smoother in the Client prediction package. It accepts `ReconciliationResult` only after authority has updated simulation, preserves continuity for bounded corrections, decays residual over caller-supplied render time, and snaps on large/untrusted corrections or a reconnect boundary. The candidate parameters are 100 ms and 250 mm. This does not change the accepted 60 Hz authority decision, protocol, replay, or server behavior; exact-commit macOS evidence and later game-specific visual/physics evaluation remain required.
