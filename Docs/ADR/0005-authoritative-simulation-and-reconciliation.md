...
WS-27 adds a fixed-size millimetre correction histogram to the project-owned prediction adapter and a resettable measurement window without changing prediction or protocol behavior. The macOS Player gate applies the frozen Regional qdisc profile symmetrically across the real Fantasy KCP path, waits through a declared warm-up, then measures the existing P95/P99 and correction-frequency budgets for 60 seconds. This closes only the local macOS Mono real-client correction gate when an exact-commit bundle passes; smoothing, prediction physics, Windows, mobile IL2CPP, production defaults, and a real Linux environment canary remain independent.
